  ...
  return (
    <div className="animate-fade-in space-y-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {/* Heatmap shows recent activity / streak */}
        <StudyHeatmap />
      </header>
      ...
      <form
        ...
        className="bg-surface p-6 rounded-2xl border border-white/5 flex flex-col md:flex-row gap-4 items-center"
      >
        <input
          type="text"
            ...
            <div className="bg-surface rounded-2xl border border-white/5 overflow-hidden card-hover h-full flex flex-col">
              ...
              <div className="relative h-40 bg-zinc-800 overflow-hidden">
                ...
                <div className="absolute bottom-2 left-4 right-4">
                  <h3 className="font-bold text-lg truncate drop-shadow-md">
                    {pl.title}
                  </h3>
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm text-zinc-400">
                    <span>
                      {pl.completedCount} / {pl.videoCount} Videos
                    </span>
                    <span>{pl.percent}%</span>
                  </div>
                  {/* replaced external ProgressBar with inline bar to avoid dependency on deleted file */}
                ...
                <div className="mt-4 pt-4 border-t border-white/5 flex items-center gap-2 text-xs text-zinc-500">
                  <Clock size={14} />
                  <span>
                    Total Duration: {Math.floor(pl.totalDurationSeconds / 3600)}
                    h {Math.floor((pl.totalDurationSeconds % 3600) / 60)}m
                  </span>
                </div>
              </div>
            </div>
      ...
      {!fetching && playlists.length === 0 && (
        <div className="text-center py-20 bg-surface/30 rounded-2xl border border-white/5 border-dashed">
          <p className="text-zinc-500">
            No playlists found. Import one to get started!
          </p>
        </div>
      )}
    </div>
